...
from remembr.captioners.qwen_captioner import Qwen25VLCaptioner
...
def memory_builder_args(args=None):
    ...
    parser.add_argument(
        "--model-path", type=str, default="Qwen/Qwen2.5-VL-7B-Instruct"
    )
...
class ROSMemoryBuilder(Node):
    def __init__(
        ...
        if "compressed" in image_topic:
            self.img_sub = self.create_subscription(
                CompressedImage, image_topic, self.img_listener_callback, 10
            )
            self.is_compressed = True
        else:
            self.img_sub = self.create_subscription(
                Image, image_topic, self.img_listener_callback, 10
            )

        if "odom" in pos_topic:
            self.pose_sub = self.create_subscription(
                Odometry, pos_topic, self.pose_listener_callback, 10
            )
        else:
            self.pose_sub = self.create_subscription(
                PoseWithCovarianceStamped, pos_topic, self.pose_listener_callback, 10
            )

        self.counter = 0

        self.captioner = Qwen25VLCaptioner(args)
    ...
    def img_listener_callback(self, img_msg):
        ...
        converted_time = float(str(stamp.sec) + "." + str(stamp.nanosec))
        data_dict = {"image": PIL_img, "time": converted_time}
        ...
        if converted_time - self.start_time >= self.segment_time:
            ...
            if len(self.pose_buffer) == 0:
                ...
                pose_dict = {"position": positions, "orientation": orientations}
            self.vila_executor.submit(
                self.process_into_db(self.image_buffer, pose_dict)
            )
